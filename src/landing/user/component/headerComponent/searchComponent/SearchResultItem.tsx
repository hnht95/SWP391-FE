...
const SearchResultItem: React.FC<SearchResultItemProps> = ({
  ...
  return (
    <button 
      key={`${index}-${isSelected}`} // Force re-render when selection changes
      onClick={onClick}
      ...
      style={{
        ...
        ...(isSelected && { 
          animation: 'selection-scale 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) forwards'
        })
      }}
    >
      ...
      {isSelected && (
        <div className="absolute inset-0"
          ...
          <div 
            ...
            style={{ 
              filter: 'drop-shadow(0 0 16px rgba(200, 230, 255, 0.9)) drop-shadow(0 0 8px rgba(255, 255, 255, 1))' 
            }}
          ></div>
          
          {/* Diamond Sparkle Points */}
          <div className="absolute top-1/4 left-1/4 w-1 h-1 bg-white rounded-full"
               style={{ animation: 'diamond-sparkle 0.8s ease-in-out 0.2s infinite' }}></div>
          <div className="absolute top-1/2 left-1/3 w-0.5 h-0.5 bg-cyan-100 rounded-full"
               style={{ animation: 'diamond-sparkle 0.6s ease-in-out 0.4s infinite' }}></div>
          <div className="absolute top-3/4 left-1/2 w-1.5 h-1.5 bg-slate-100 rounded-full"
               style={{ animation: 'diamond-sparkle 1s ease-in-out 0.1s infinite' }}></div>
          <div className="absolute top-1/3 right-1/4 w-0.5 h-0.5 bg-white rounded-full"
               style={{ animation: 'diamond-sparkle 0.7s ease-in-out 0.3s infinite' }}></div>
        </div>
      )}
      
      {/* Cinematic glow effect */}
      <div className={`absolute inset-0 bg-gradient-to-r ${classes.glow} rounded-lg transition-all duration-400 ease-cinematic ${
        isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-60'
      }`}
      style={{
        ...(isSelected && { animation: 'selection-glow 0.5s ease-cinematic forwards' })
      }}></div>
