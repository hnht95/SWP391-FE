  ...
  return (
    <>
      <div
        className={`${
          variant === "inline"
            ? "w-full bg-transparent z-[110]"
            : "w-full sticky top-[80px] md:top-[88px] z-60 bg-black/50 backdrop-blur-md transition-transform duration-700 ease-in-out will-change-transform select-none"
      ...
      >
        <nav className="w-full px-4">
          <ul
            className={`flex list-none items-center justify-center gap-6 md:gap-10 text-sm md:text-base text-white ${
              variant === "inline" ? "py-0" : "py-3 md:py-4"
            }`}
          ...
          >
            ...
            {navigationPaths.contactus && (
              <li onMouseEnter={() => setHoveredItem(4)}>
                <Link
                  className={`no-underline font-medium transition-all duration-300 cursor-pointer ${
                    hoveredItem !== null && hoveredItem !== 4
                      ? "text-white/50"
                      : "text-white"
                  } ${
                    hoveredItem === 4
